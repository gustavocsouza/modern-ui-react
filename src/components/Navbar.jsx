...
export default function Navbar({ scrolled }) {
  ...
  return (
    <nav className={`fixed top-0 w-full z-50 transition-all duration-300 border-slate-800 ${
          scrolled 
          ? "bg-slate-950/80 backdrop-blur-lg border-b" 
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-14 sm:h-16 md:h-20">
          ...
          <button 
            ...
            onClick={() => setMobileMenuIsOpen(previousState => !previousState)}
          >
            { mobileMenuIsOpen 
              ? <XIcon className="w-5 h-5 sm:w-5 sm:h-6"/>
              : <Menu className="w-5 h-5 sm:w-5 sm:h-6"/>
            }
          </button>
        </div>
      </div>
